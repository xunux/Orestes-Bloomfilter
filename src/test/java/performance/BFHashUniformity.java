...
public class BFHashUniformity {
    ...
    public static enum Randoms {
        UUIDS("UUIDs", new Function<Integer, String>() {
            @Override
            public String apply(Integer i) {
                return UUID.randomUUID().toString();
            }
        }), INTS("increasing integers", new Function<Integer, String>() {
            @Override
            public String apply(Integer i) {
                return i + "";
            }
        }), ID("incresing String IDs", new Function<Integer, String>() {
            @Override
            public String apply(Integer i) {
                return "object" + i;
            }
        }), RANDINTS("random integers", new Function<Integer, String>() {
            @Override
            public String apply(Integer i) {
                return new Random(i).nextInt() + "";
            }
        }), RANDWORDS("random words", new Function<Integer, String>() {
            @Override
            public String apply(Integer i) {
                return RandomStringUtils.random(new Random(i).nextInt(100));
            }
        }), BYTES("increasing bytes", new Function<Integer, String>() {
            @Override
            public String apply(Integer i) {
        ...
        public List<List<byte[]>> generate(int hashesPerRound, int rounds) {
            return generate(hashesPerRound, rounds, generator, new Function<String, byte[]>() {
                @Override
                public byte[] apply(String t) {
                    return Randoms.toBytes(t);
                }
            });
        }

        private static <T, K> List<List<K>> generate(int hashesPerRound, int rounds, Function<Integer, T> generator,
                Function<T, K> converter) {
            List<List<K>> hashData = new ArrayList<List<K>>(rounds);
            for (int j = 0; j < rounds; j++) {
                hashData.add(new ArrayList<K>(hashesPerRound));
            }
            for (int i = 0; i < rounds; i++) {
