...
public class RedisCBFTest {
    ...
    public void concurrencyTests() throws InterruptedException {
        ...
        final int k = 10;
        int threads = 4;
        final AtomicBoolean false_negative = new AtomicBoolean(false);
        final List<String> real = Collections.synchronizedList(new ArrayList<String>(objects));
        ...
        for (int i = 0; i < threads; i++) {
            ...
            Runnable run = new Runnable() {
                @Override
                public void run() {
                    CountingBloomFilter<String> filter = new FilterBuilder(m, k).name(name).redisBacked(true)
